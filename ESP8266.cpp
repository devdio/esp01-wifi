...
bool ESP8266::recvFind(String target, uint32_t timeout)
{
    ...
    //if(h_serial) {
    //    h_serial->println("recvFind >>>");
    //    h_serial->println(data_tmp.c_str());
    //    h_serial->println("<<<");
    //}
...
//-------------------------------------------------------------------------------
//-------------------------------------------------------------------------------
bool ESP8266::get(const String& host, const String& path, uint32_t port) {
    ...
    uint32_t len = path.length() + host.length();
    
    for(uint8_t i = 0; i < nGetRequest; ++i) {
        len += getRequest[i].length();
    }
    ...
    for(uint8_t i = 0;  i < nGetRequest;  ++i) {
        for(uint32_t j = 0; j < getRequest[i].length(); ++j) {
            ...
            m_puart->write(getRequest[i][j]);
            //if(h_serial) h_serial->print(getRequest[i][j]);
        }
        for(uint32_t j = 0; i == 0 && j < path.length(); ++j) {
            if(++sentLen % 64 == 0) delay(20);
            m_puart->write(path[j]);
            //if(h_serial) h_serial->print(path[j]);
        }
        for(uint32_t j = 0; i == 1 && j < host.length(); ++j) {
            if(++sentLen % 64 == 0) delay(20);
            m_puart->write(host[j]);
            //if(h_serial) h_serial->print(host[j]);
        }
    }

    //debugPrint();
    //if(recvFind("SEND OK", 10000)) {
    //    if(h_serial) h_serial->println("SEND OK...");
    //} else {
    //    if(h_serial) h_serial->println("SEND ERR...");
    //}
...
bool ESP8266::post(const String& host, const String& path, const String& body,
    ...
    for(uint8_t i = 0; i < nPostRequest; ++i) {
        len += postRequest[i].length();
    }
    ...
    uint32_t sentLen = 0;
    for(uint8_t i = 0; i < nPostRequest; ++i) {
        for(uint32_t j = 0; j < postRequest[i].length(); ++j) {
            if(++sentLen % 64 == 0) delay(20); // Some Arduino's default serial buffer size is 64 bytes (Wait for it to be empty again.)
            m_puart->write(postRequest[i][j]);
            //if(h_serial) h_serial->print(postRequest[i][j]);
        }
        for(uint32_t j = 0; i == 0 && j < path.length(); ++j) {
            if(++sentLen % 64 == 0) delay(20);
            m_puart->write(path[j]);
        ...
        for(uint32_t j = 0; i == 2 && j < contentType.length(); ++j) {
            if(++sentLen % 64 == 0) delay(20);
            m_puart->write(contentType[j]);
            //if(h_serial) h_serial->print(contentType[j]);
        }
        for(uint32_t j = 0; i == 4 && j < body.length(); ++j) {
            if(++sentLen % 64 == 0) delay(20);
            m_puart->write(body[j]);
            //if(h_serial) h_serial->print(body[j]);
        }
    }
...
void ESP8266::debugPrint() {
    if(h_serial) {
        ...
        while (millis() - start < 10000) {
            while(m_puart->available() > 0) {
                c = m_puart->read(); // 1 byte
                if(c == '\0') continue;
                buf += c;
            }
        }
        h_serial->println(buf.c_str());
        h_serial->println("---###");
    }
}
